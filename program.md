...
- Fixed **training time budget: 30 minutes**
- Command:
  ```
  uv run train.py
  ```
...
- Modify `train.py`
- Modify `data.py`
- Install additional packages (e.g. medical imaging tools)
...
The evaluation function is generated **ONLY ONCE unless user explicitly ask you to the change it**.
...
After each epoch, save current model weight as `checkpoints/last.pth` and conduct evaluation. Track the best-performed model based on **primary_metric** through the current 30-min running, and save the best model as `checkpoints/best.pt`. Improvement rule:
...
After 30-min training finishes, print the best validation results as follows:
...
**Timeout**: Each experiment should take ~30 minutes total (with startup and eval excluded). If a run exceeds 40 minutes, kill it and treat it as a failure (discard and revert).
...
As an example use case, a user might leave you running while they sleep. If each experiment takes you ~30 minutes then you can run approx 2/hour, for a total of about 18 over the duration of the average human sleep. The user then wakes up to experimental results, all completed by you while they slept!